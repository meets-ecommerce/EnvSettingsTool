 ...
 * - type (html or text
 */
class Est_Handler_Magento_CoreCustomVariable extends Est_Handler_Magento_AbstractDatabase
{
    /**
        ...
        if ($value !== false) {
            if ($params['type'] === 'text') {
                if ($value['plain_value'] == $this->value) {
                    $this->addMessage(
                        new Est_Message(sprintf('Value "%s" is already in place. Skipping.', $value['plain_value']), Est_Message::SKIPPED)
                    );
                } else {
                    $this->_processUpdate(
                ...
                if ($value['html_value'] == $this->value) {
                    $this->addMessage(
                        new Est_Message(sprintf('Value "%s" is already in place. Skipping.', $value['html_value']), Est_Message::SKIPPED)
                    );
                } else {
                    $this->_processUpdate(
