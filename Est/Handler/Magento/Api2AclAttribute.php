 ...
 */
class Est_Handler_Magento_Api2AclAttribute extends Est_Handler_Magento_AbstractDatabase
{
        ...
        if ($action == Est_Handler_Magento_AbstractDatabase::ACTION_DELETE) {
            ...
            if ($firstRow) {
                if ($firstRow['allowed_attributes'] === $this->value) {
                    $this->addMessage(new Est_Message('Attribute is already in place. Skipping.'), Est_Message::SKIPPED);
                } else {
                    if ($resource_id === 'all') {
